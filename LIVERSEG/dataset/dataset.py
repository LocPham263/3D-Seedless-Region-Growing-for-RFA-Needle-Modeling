"""
dataset cho train và val
"""

import os
import sys
sys.path.append(os.path.join(sys.path[0])[0])
import random
import numpy as np
...
class Dataset(dataset):
    ...
    def __getitem__(self, index):
        ...
        # giảm giá trị pixel trong ảnh
        ct_array = ct_array.astype(np.float32)
        ct_array = ct_array / 200

        start_slice = random.randint(0, ct_array.shape[0] - para.size_block)
        end_slice = start_slice + para.size_block - 1
